...
def run():
    ...
    5. 限价单只用来开仓，委托单只用来平仓
    6. 开仓如果只成交部分挂单，则等这笔交易完成（止盈或者闪电平仓止损），再进行新的交易计划
    """
    ...
    # 交易是否正在进行，如果是则等待结束该轮交易，再进行下一轮
    transaction_is_ongoing = False
    ...
    transaction_is_ongoing = limit_holding_on_trend_count != 0
    ...
    if not transaction_is_ongoing and limit_pending_on_trend_count > 0:
