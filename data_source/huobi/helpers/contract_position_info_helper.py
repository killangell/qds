...
class ContractPositionInfoHelper:
    ...
    def parse_return_value(self, ret):
        val = None
        if ret:
            val = {'status': ru.get_status(ret),
                   'ts': ru.get_ts(ret)}

            stripped_data = []
            size = ru.get_data_list_size(ret)
            if size > 0:
                for i in range(0, size):
                    symbol = ru.get_data_item_field(i, 'symbol', ret)
                    contract_type = ru.get_data_item_field(i, 'contract_type', ret)
                    volume = ru.get_data_item_field(i, 'volume', ret)
                    available = ru.get_data_item_field(i, 'available', ret)
                    cost_open = ru.get_data_item_field(i, 'cost_open', ret)
                    cost_hold = ru.get_data_item_field(i, 'cost_hold', ret)
                    lever_rate = ru.get_data_item_field(i, 'lever_rate', ret)
                    direction = ru.get_data_item_field(i, 'direction', ret)
                    stripped_item = {symbol, contract_type, volume, available, cost_open,
                                     cost_hold, lever_rate, direction}
                    stripped_data.append(stripped_item)
            val['data'] = stripped_data
        return val
    """
    """
    请求参数
